...
* **관측 행위의 부작용**: 디버깅을 위해 넣은 UART printf가 SPI 타이밍을 밀리게 해 오류를 유발한 경험을 통해, 관측 행위 자체가 시스템에 영향을 준다는 점을 실감. 임베디드 환경에서 비침습적(non-intrusive) 디버깅 방법의 중요성을 체감.
...
* **UVM 검증 환경 설계**: Scoreboard의 참조 데이터를 올바른 소스에서 가져오는 것이 검증 신뢰성의 핵심임을 확인. Monitor의 감시 시점(AXI W채널 vs busy 전환)을 명확히 정의하지 않으면 타이밍 불일치로 인해 검증 자체가 무의미해질 수 있음을 경험.
